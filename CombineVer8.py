...
mydb = mysql.connector.connect(
    ...
    database = "heroku_b7c2cd340093dfb",
    user = "b69ea4764a95ef",
    password = "bbb995fd"
)
...
sql_food_query = pd.read_sql('SELECT * FROM fooddata', mydb)

food_df = pd.DataFrame(sql_food_query, columns = ['restaurant_id','name', 'address', 'cuisine'])


sql_users_query = pd.read_sql('SELECT * FROM tblusers', mydb)

users_df = pd.DataFrame(sql_users_query, columns = ['Id','telehandle'])


sql_ratings_query = pd.read_sql('SELECT * FROM tblratings', mydb)

ratings_df = pd.DataFrame(sql_ratings_query, columns = ['user_id','rest_id','rating'])
...
def start(update: Update, context: CallbackContext) -> int:
    ...
    USERS_DATA = list(map(lambda x : x[1], mycursor.fetchall()))
    if user_id not in USERS_DATA:
        mycursor.execute("SELECT * FROM tblusers")
        SQL_USER_DATA = mycursor.fetchall()
        newuserid = len(SQL_USER_DATA) + 1
        mycursor.execute("INSERT INTO tblusers (Id, telehandle) VALUES (%s, %s)", (newuserid, user_id))
        mydb.commit()
...
def eleven(update: Update, context: CallbackContext) -> int:
    ...
    real_user = list(filter(lambda x : x[1] == update.message.from_user.username, SAMPLE_USERS))[0][0]
...
def twelve(update: Update, context: CallbackContext) -> int:
    ...
    for i in current_users:
        ...
        mycursor.execute("SELECT * FROM tblusers")
        SAMPLE_USERS = mycursor.fetchall()
        real_user = list(filter(lambda x : x[1] == i, SAMPLE_USERS))[0][0]
        print(real_user)
        mycursor.execute("SELECT * FROM tblratings")
        SAMPLE_RATINGS = mycursor.fetchall()
        sampleratinglist = list(filter(lambda x : x[0] == real_user and x[1] == rest_idid, SAMPLE_RATINGS))
        print(sampleratinglist)
        if len(sampleratinglist) != 0:
            mycursor.execute("DELETE FROM tblratings WHERE user_id = %s AND rest_id = %s", (real_user, rest_idid))
        mycursor.execute("INSERT INTO tblratings (user_id, rest_id) VALUES (%s, %s)", (real_user, rest_idid))
        mydb.commit()
    keyboard = [
    ...
    context.bot.send_message(chat_id = update.effective_chat.id, text = "Please rate the restaurant", reply_markup = ReplyKeyboardMarkup(keyboard))
    return THIRD    
...
def end(update: Update, context: CallbackContext) -> int:
    ...
    """
    if type(update.message.text) == int and get_confidence(f"{update.message.text}") != 'High' or int(update.message.text[:2]) > 80 or len(update.message.text) != 6:
    ...
    if len(BUDGET_SELECTED) == get_chat_member_count(token, update.effective_chat.id) - 1:
        try:
            ...
            new_result = list(filter(lambda x : final_cuisine == x[5], SAMPLE_DATABASE))
            new_result = list(filter(lambda x : final_budget == x[6], new_result))
            new_result = sorted(new_result, key = lambda x : getHaversineDistance(final_location, get_lat_lon(x[4])))
            context.bot.send_message(chat_id=update.effective_chat.id, text = f"{new_result[0][1]}" + "\n" + f"Address: {new_result[0][3]}")
            LOCATION.clear()
            CUISINES_SELECTED.clear()
            BUDGET_SELECTED.clear()
            keyboard = [
            ...
            reply_markup = InlineKeyboardMarkup(keyboard)
            # Send message with text and appended InlineKeyboard
            REST_ID.append(new_result[0][0])
            update.message.reply_text("Did you visit the restaurant?", reply_markup=reply_markup)
            return FIRST
        except IndexError:
...
def messageHandler(update: Update, context: CallbackContext):
    ...
    if ratingone in update.message.text:
        sql = "UPDATE tblratings SET rating = 1 WHERE user_id = %s AND rest_id = %s"
        mycursor.execute(sql, (real_user, rest_idid))
        mydb.commit()
        count.append("Yes")
        if len(count) == get_chat_member_count(token, update.effective_chat.id) - 1:
            count.clear()
            return ConversationHandler.END
    elif ratingtwo in update.message.text:
        sql = "UPDATE tblratings SET rating = 2 WHERE user_id = %s AND rest_id = %s"
        mycursor.execute(sql, (real_user, rest_idid))
        mydb.commit()
        count.append("Yes")
        if len(count) == get_chat_member_count(token, update.effective_chat.id) - 1:
        ...
        sql = "UPDATE tblratings SET rating = 3 WHERE user_id = %s AND rest_id = %s"
        mycursor.execute(sql, (real_user, rest_idid))
        mydb.commit()
        count.append("Yes")
        if len(count) == get_chat_member_count(token, update.effective_chat.id) - 1:
        ...
        sql = "UPDATE tblratings SET rating = 4 WHERE user_id = %s AND rest_id = %s"
        mycursor.execute(sql, (real_user, rest_idid))
        mydb.commit()
        count.append("Yes")
        if len(count) == get_chat_member_count(token, update.effective_chat.id) - 1:
        ...
        sql = "UPDATE tblratings SET rating = 5 WHERE user_id = %s AND rest_id = %s"
        mycursor.execute(sql, (real_user, rest_idid))
        mydb.commit()
        count.append("Yes")
        if len(count) == get_chat_member_count(token, update.effective_chat.id) - 1:
